...
public class Mail {	
	...
	public boolean setCopyTo(String copyto) {
		if (copyto == null)
			return false;
		try {
			String copytoMails[] = copyto.split(";");
			Address[] mailAdresses = new InternetAddress[copytoMails.length];
			for(int i=0; i<copytoMails.length; i++){
				mailAdresses[i] =  new InternetAddress(copytoMails[i]);
				
				mimeMsg.setRecipients(Message.RecipientType.CC, mailAdresses);
			}
